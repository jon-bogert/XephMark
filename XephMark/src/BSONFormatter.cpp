...
Node XephMark::BSONFormatter::LoadFromFile(const std::filesystem::path& path) const
{
...
Node XephMark::BSONFormatter::Read(const std::vector<uint8_t>& content) const
{
...
bool XephMark::BSONFormatter::SaveToFile(const Node& node, const std::filesystem::path& path) const
{
    std::vector<uint8_t> content = Dump(node);
...
std::vector<uint8_t> XephMark::BSONFormatter::Dump(const Node& node) const
{
    std::vector<uint8_t> result;
    json content;
    Export(content, node);
    result = json::to_bson(content);
}
